...
def send_message(message):
    try:
        ...
        if message.text == '/772077':
            ...
            wind_3 = w.wind()['deg']
            wind_4 = []
            logging.info(f'Отправлен ответ пользователю {message.chat.id}: '
                         f'погода в городе {message.text}: '
                         f'{temperature} градусов, '
                         f'ветер {wind} м/c, '
                         f'влажность {humidity_1}, '
                         f'давление {pressure_dict["press"]} мм.рт.ст, '
                         f'видимость {visibility}, '
                         f'направление ветра {wind_3} градусов, '
                         f'сила порывов ветра {wind_2} м/c.')
            if 0 < wind_3 < 23:
                wind_4.append('С-СВ')
            elif 22 < wind_3 < 46:
                wind_4.append('СВ')
